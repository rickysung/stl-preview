...
//  >::value, int> = 0>
//  constexpr explicit compressed_pair(T2&& t, compressed_pair_empty_t)
...
//  >::value, int> = 0>
//  constexpr explicit compressed_pair(compressed_pair_empty_t, U2&& u)
//      : second_base(std::forward<U2>(u)) {}
  ...
  template<typename Dummy = void, std::enable_if_t<conjunction<std::is_void<Dummy>,
      ...
      std::is_copy_constructible<U>,
      disjunction<
          negation<std::is_convertible<const T&, T>>,
